...
var snap = require('./deps/snapsvglite');
var easings = require('./deps/easings');
var helpers = require('./deps/helpers');
...
function SVGMorpheus(element, options, callback) {
  ...
  this._fnTick = function (timePassed) {
    ...
    if (progress < 1) {
      that._rafid = helpers._requestAnimationFrame(that._fnTick);
    } else {
      that._animationEnd();
    }
  };
...
SVGMorpheus.prototype = {
  ...
  _setupAnimation: function (toIconId) {
    if (!!toIconId && !!this._icons[toIconId]) {
      this._toIconId = toIconId;
      this._startTime = undefined;
      var i, len, j, len2;
      this._fromIconItems = _.clone(this._curIconItems);
      this._toIconItems = _.clone(this._icons[toIconId].items);
      ...
      for (i = 0; i < maxNum; i++) {
        ...
        if (!this._fromIconItems[i]) {
          if (!!this._toIconItems[i]) {
            toBB = snap.curvePathBBox(snap.path2curve(this._toIconItems[i].path));
        ...
        if (!this._toIconItems[i]) {
          if (!!this._fromIconItems[i]) {
            toBB = snap.curvePathBBox(snap.path2curve(this._fromIconItems[i].path));
      ...
      for (i = 0; i < maxNum; i++) {
        ...
        var curves = snap.path2curve(this._fromIconItems[i].path, this._toIconItems[i].path);
        fromIconItem.curve = curves[0];
        toIconItem.curve = curves[1];

        // Normalize from/to attrs
        var attrsNorm = helpers.styleToNorm(this._fromIconItems[i].attrs, this._toIconItems[i].attrs);
        fromIconItem.attrsNorm = attrsNorm[0];
        toIconItem.attrsNorm = attrsNorm[1];
        fromIconItem.attrs = helpers.styleNormToString(fromIconItem.attrsNorm);
        toIconItem.attrs = helpers.styleNormToString(toIconItem.attrsNorm);

        // Normalize from/to style
        var styleNorm = helpers.styleToNorm(this._fromIconItems[i].style, this._toIconItems[i].style);
        fromIconItem.styleNorm = styleNorm[0];
        toIconItem.styleNorm = styleNorm[1];
        fromIconItem.style = helpers.styleNormToString(fromIconItem.styleNorm);
        toIconItem.style = helpers.styleNormToString(toIconItem.styleNorm);

        // Calculate from/to transform
        toBB = snap.curvePathBBox(toIconItem.curve);
        toIconItem.trans = {
          'rotate': [0, toBB.cx, toBB.cy]
        };
      ...
      this._curIconItems = _.clone(this._fromIconItems);
    }
  },

  _updateAnimationProgress: function (progress) {
    progress = easings[this._easing](progress);

    var i, j, k, len;
    // Update path/attrs/transform
    for (i = 0, len = this._curIconItems.length; i < len; i++) {
      this._curIconItems[i].curve = helpers.curveCalc(this._fromIconItems[i].curve, this._toIconItems[i].curve, progress);
      this._curIconItems[i].path = snap.path2string(this._curIconItems[i].curve);

      this._curIconItems[i].attrsNorm = helpers.styleNormCalc(this._fromIconItems[i].attrsNorm, this._toIconItems[i].attrsNorm, progress);
      this._curIconItems[i].attrs = helpers.styleNormToString(this._curIconItems[i].attrsNorm);

      this._curIconItems[i].styleNorm = helpers.styleNormCalc(this._fromIconItems[i].styleNorm, this._toIconItems[i].styleNorm, progress);
      this._curIconItems[i].style = helpers.styleNormToString(this._curIconItems[i].styleNorm);

      this._curIconItems[i].trans = helpers.transCalc(this._fromIconItems[i].trans, this._toIconItems[i].trans, progress);
      this._curIconItems[i].transStr = helpers.trans2string(this._curIconItems[i].trans);
    }
  ...
  to: function (iconId, options, callback) {
    if (iconId !== this._toIconId) {
      ...
      helpers._cancelAnimationFrame(this._rafid);
      ...
      this._rafid = helpers._requestAnimationFrame(this._fnTick);
    }
  }
